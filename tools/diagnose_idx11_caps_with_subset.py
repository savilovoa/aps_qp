...
TARGET_IDX = 11  # внешний idx продукта
...
def main() -> None:
    ...
    base_status, base_status_str = run_long_simple_once(
        ...
        cleans,
        remains,
        title="baseline LONG_SIMPLE (no cap for idx=11)",
        html_name="res_idx11_baseline.html",
    )
    ...
    # --- Прогон с капом plan_days+2 для idx=11 ---
    ...
    cap_status, cap_status_str = run_long_simple_once(
        ...
        cleans,
        remains,
        title=f"LONG_SIMPLE cap idx={TARGET_IDX} plan+2 (cap_days={cap_days})",
        html_name="res_idx11_cap_plan_plus_2.html",
    )

    if cap_status not in (cp_model.OPTIMAL, cp_model.FEASIBLE):
        print("Модель с капом plan+2 для idx=11 НЕ выполнима (см. лог simple_constraints_p11.log).")

    # --- Повтор с отключённой монотонностью по C[p,d] ---
    print("\n=== LONG_SIMPLE with cap idx=11 and SIMPLE_USE_MONOTONE_COUNTS=False ===")
    settings.SIMPLE_USE_MONOTONE_COUNTS = False
    # те же капы и дамп
    cap_status2, cap_status_str2 = run_long_simple_once(
        data,
        machines,
        products,
        cleans,
        ...
        title=(
            ...
            f"SIMPLE_USE_MONOTONE_COUNTS=False (cap_days={cap_days})"
        ),
        html_name="res_idx11_cap_plan_plus_2_no_monotone.html",
    )
    print(
        f"  status={cap_status2} ({cap_status_str2}) "
        f"при отключённой монотонности по C[p,d]"
    )
