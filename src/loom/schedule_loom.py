...
def create_model(remains: list, products: list, machines: list, max_daily_prod_zero: int, count_days: int):
...
def schedule_loom_calc(remains: list, products: list, machines: list, max_daily_prod_zero: int, count_days: int) -> LoomPlansOut:
    try:
        ...
        model, jobs, product_counts, proportion_objective_terms = create_model(
            remains=remains, products=products, machines=machines, max_daily_prod_zero=max_daily_prod_zero,
...
def schedule_loom_calc_2() -> LoomPlansOut:
    try:
        ...
        for m in range(num_machines):
            for d in range(2, num_days):
                ...
                model.AddBoolAnd([jobs[m, d - 1] == PRODUCT_ZERO, jobs[m, d - 2] == PRODUCT_ZERO]).OnlyEnforceIf(
                    two_day_zero)
                model.AddBoolOr([jobs[m, d - 1] != PRODUCT_ZERO, jobs[m, d - 2] != PRODUCT_ZERO]).OnlyEnforceIf(
                    two_day_zero.Not())
