...
def create_model_simple(remains: list, products: list, machines: list, cleans: list,
    ...
    for m in all_machines:
        for d in range(0, num_days):
            if d == 0:
                pred_d = initial_products[m] if initial_products[m] > 0 else jobs[m, d]
            else:
                pred_d = jobs[m, d - 1]
            ch = model.NewBoolVar(f"change_simple_{m}_{d}")
            change_bools[m, d] = ch
            # Определяем факт смены продукта между днями d-1 и d.
            model.Add(jobs[m, d] != pred_d).OnlyEnforceIf(ch)
            model.Add(jobs[m, d] == pred_d).OnlyEnforceIf(ch.Not())
